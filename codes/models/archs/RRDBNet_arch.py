...
class RRDBNet(nn.Module):
    ...
    def forward(self, x):
        # for onnx export
        # To work with TensorRT, You should also change upsample to fixed_size
        # sh = torch.tensor(fea.shape)
        # size=(int(sh[2]*scale), int(sh[3]*scale))
        self.x = x
        fea = self.conv_first(self.x)
        ...
        self.out = self.conv_last(self.lrelu(self.HRconv(fea)))
